...
이번 분석에서는 다양한 머신러닝 모델을 활용하여 타이타닉 승객의 생존 여부를 예측했습니다. **결측값 처리**, **범주형 변수 인코딩**, **FamilySize 파생 변수 생성**, **Cabin 유무 이진화** 등의 전처리를 통해 비교적 간단한 구조의 모델들도 효과적인 성능을 보였습니다.

###  모델 성능 비교

- **로지스틱 회귀 (Logistic Regression)**  
  - AUC: 0.84 / 정확도 약 80%  
  - 단순한 선형 모델임에도 불구하고 매우 안정적인 성능을 보였습니다.
  
- **랜덤 포레스트 (Random Forest)**  
  - AUC: 0.83 / 정확도 약 81%  
  - 높은 재현율로 생존자를 잘 예측했으며, 특성 중요도 분석에서도 해석이 용이합니다.
  
- **XGBoost**  
  - AUC: 0.81 / 정확도 약 80%  
  - 랜덤 포레스트에 근접한 성능을 보였으나 생존자 탐지율은 다소 낮았습니다.
  
- **MLP 신경망**  
  - AUC: 0.82 / 정확도 약 80%  
  - 비선형 모델임에도 불구하고 구조적 복잡성에 비해 트리 기반 모델보다 우위는 없었습니다.
  
- **의사결정나무 (Decision Tree)**  
  - AUC: 0.75 / 정확도 약 76%  
  - 단일 트리의 한계로 인해 가장 낮은 성능을 기록했습니다.

###  혼동 행렬 분석

- 랜덤 포레스트는 생존자(True Positive)를 50명으로 가장 많이 잡아냈고, 거짓 음성(False Negative)을 가장 낮췄습니다(19건).
- 로지스틱 회귀와 의사결정나무는 각각 생존자를 48명, 47명 예측했고, 거짓 음성은 21~22건이었습니다.
- XGBoost와 MLP는 각각 생존자를 45명 예측했으며, 모두 24건의 생존자 예측 실패가 있었습니다.

###  주요 영향 변수

랜덤 포레스트의 특성 중요도 분석 결과:

- **Fare (요금)**, **Sex (성별)**, **Age (나이)**가 생존 예측에 가장 큰 기여를 했습니다.
- 이 세 변수는 타이타닉 사건의 역사적 맥락(여성과 아이 우선 구조, 고급 객실일수록 높은 생존율)과 매우 잘 일치합니다.
- **HasCabin**, **FamilySize** 등의 변수도 중간 수준의 설명력을 보였고, **Embarked**, **Pclass** 등의 더미 변수는 상대적으로 낮은 기여도를 보였습니다.

---

###  종합적 시사점

- 단순한 모델(예: 로지스틱 회귀)도 잘 정제된 데이터와 적절한 전처리를 통해 **강력한 예측 성능**을 낼 수 있습니다.
- 복잡한 모델(랜덤 포레스트, XGBoost)은 약간의 성능 향상을 제공하지만, 그 해석 가능성과 적용 목적을 고려해 **모델 선택의 균형**이 중요합니다.
- 본 사례는 머신러닝 모델이 **도메인 지식과 일치하는 의미 있는 패턴을 효과적으로 학습**할 수 있음을 보여줍니다.
